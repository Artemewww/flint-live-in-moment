...
-- 9c. RLS не трогаем.
--
-- История: бот молча терял состояние диалогов, потому что в env лежал НЕ
-- service_role ключ (он единственный обходит RLS). Соблазн «отключить RLS»
-- на служебных таблицах — ложный путь: он открывает базу наружу.
-- Правильное решение — положить в SUPABASE_SERVICE_ROLE_KEY настоящий
-- service_role / secret key. Сделано 10.07.2026.
--
-- Проверка: GET /api/events?action=health должен показать *:WRITE → ok.
-- Зонд проверяет именно ЗАПИСЬ: при включённом RLS select молча возвращает
-- пусто и выглядит исправным.
-- ─────────────────────────────────────────────────────────────
