...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const prompt =
      ...
      `Верни JSON: массив program из ${Number(body?.count) >= 1 && Number(body?.count) <= 20 ? `РОВНО ${Number(body.count)}` : current.length ? `РОВНО ${current.length}` : '5–9'} пунктов (каждый — короткая строка шага программы, можно со временем).`;
    const parsed = await genJSON(ai, prompt, {
      type: Type.OBJECT,
