...
/**
 * Чек-лист для кемпинга — инлайн вместо import('../_lib/camping-checklist'):
 * Vercel не бандлит файлы из папок с «_» в рантайме serverless-функции,
 * импорт ронял MODULE_NOT_FOUND на загрузке МОДУЛЯ — то есть на КАЖДОМ
 * апдейте вебхука (см. тот же фикс для group-ban, коммит "webhook 500").
 * Копия для фронта — src/data/campingChecklist.ts, правь оба.
 */
interface ChecklistGroup { title?: string; items: string[]; }
interface ChecklistSection { key: string; emoji: string; title: string; groups: ChecklistGroup[]; tip?: string; }
const CAMPING_CHECKLIST: ChecklistSection[] = [
  {
    key: 'camp', emoji: '⛺', title: 'Лагерь',
    tip: 'Выбирайте лёгкое и компактное снаряжение — сэкономит место и силы в дороге.',
    groups: [{ items: [
      'Палатка (двухслойная)', 'Колышки, растяжки, молоточек', 'Шатёр', 'Тент (от солнца/дождя)',
      'Гамак', 'Шезлонг', 'Матрас самонадувающийся', 'Спальные мешки', 'Туристический коврик (пенка)',
      'Плед для пикника', 'Туристические фонари (2 шт)', 'Налобный фонарь', 'Гирлянда на батарейках',
      'Поджопник (сидушка)', 'Органайзер подвесной', 'Органайзер-шкафчик', 'Туристическая верёвка',
  ...
  },
  {
    ...
    groups: [
      { title: 'Оборудование', items: [
        'Газовая горелка + баллоны (2 шт)', 'Стол туристический', 'Стулья складные', 'Тренога для котелка',
        'Котелок', 'Сковородка', 'Кастрюли', 'Решётка для гриля', 'Шампуры',
        'Набор туристической посуды и контейнеров', 'Разделочная доска', 'Ножи (2 шт)',
        'Открывашка / штопор', 'Термосы',
      ] },
      { title: 'Хранение и готовка', items: [
        'Бутыли 5 л для тех. воды', 'Канистра с краником', 'Пакеты', 'Аккумуляторы холода',
      ] },
      { title: 'Расходники', items: [
        'Бумажные полотенца', 'Кухонное полотенце', 'Влажные салфетки', 'Тряпка', 'Губка', 'Фольга',
        'Пищевая плёнка', 'Zip-пакеты', 'Газета для розжига', 'Нитки', 'Соль, перец',
        'Спички / зажигалка / огниво', 'Жидкость для розжига', 'Масло подсолнечное',
        'Средство для мытья посуды', 'Спрей от комаров', 'Спрей от комаров и клещей',
        'Самонагревающиеся пакеты для еды', 'Химические грелки',
      ] },
    ],
  },
  {
    ...
    groups: [{ items: [
      'Яйца', 'Колбаса', 'Сыр', 'Макароны', 'Гречка', 'Рис', 'Консервы (тушёнка, паштет)',
      'Готовая каша с мясом', 'Свежие овощи', 'Фрукты', 'Хлебцы', 'Чай, кофе, сахар', 'Питьевая вода',
    ] }],
  },
  {
    key: 'clothes', emoji: '👕', title: 'Одежда',
    groups: [{ items: [
      'Футболки', 'Термобельё', 'Штаны', 'Шорты', 'Флиска / толстовка', 'Куртка',
      'Носки (обычные и тёплые)', 'Нижнее бельё', 'Купальник / плавки', 'Кроссовки', 'Сланцы / кроксы',
      'Дождевик (2 шт)', 'Панама / кепка',
    ] }],
  },
  {
    key: 'hygiene', emoji: '🧼', title: 'Гигиена',
    groups: [{ items: [
      'Полотенце банное', 'Полотенце для рук', 'Кухонное полотенце', 'Туалетная бумага', 'Крем от солнца',
      'Крем после солнца', 'Умывалка', 'Влажные салфетки', 'Линзы', 'Очки солнцезащитные',
      'Очки для зрения', 'Зубная щётка', 'Зубная паста', 'Шампунь и мыло',
      'Переносной душ / бутылка с насадкой', 'Репеллент от комаров и клещей', 'Средство от укусов',
      'Женская гигиена', 'Щётка для волос', 'Зеркальце',
    ] }],
  },
  {
    key: 'tools', emoji: '🛠', title: 'Инструменты и техника',
    groups: [{ items: [
      'Складной нож / мультитул', 'Батарейки', 'Карабины', 'Пускозарядное устройство',
      'Жидкости для авто', 'Ремкомплект', 'Колонка для музыки', 'Пауэрбанки (не менее 2 шт)',
      'Туристическая пила', 'Топор', 'Рабочие перчатки', 'Домкрат', 'Насос', 'Веник',
    ] }],
  },
  {
    key: 'medkit', emoji: '💊', title: 'Аптечка',
    groups: [{ items: [
      'Антигистаминное', 'Обезболивающее', 'Жаропонижающее', 'Энтеросгель / Полисорб', 'Пантенол',
      'Фенистил', 'Пластырь, бинт, вата', 'Хлоргексидин, перекись, спирт', 'Средство от укусов',
      'Эластичный бинт', 'Пинцет для клещей', 'Лекарства личные',
    ] }],
  },
  {
    key: 'fun', emoji: '🎸', title: 'Досуг',
    groups: [{ items: ['Настольные игры и карты', 'Книги', 'Гитара (или другой инструмент)', 'Колонка'] }],
  },
];
...
function checklistTotal(): number {
  return CAMPING_CHECKLIST.reduce((n, s) => n + s.groups.reduce((k, g) => k + g.items.length, 0), 0);
}
...
function eventCard(ev: any): string {
  ...
  return (
    ...
    `${aud}\n` +
    // Условия, меняющие ожидания от события. Короткими строками в карточке, а
    // не за кнопкой: телефон и связь человек планирует ДО поездки.
    (ev.logistics?.detox ? `📵 Цифровой детокс — без телефонов\n` : '') +
    (ev.logistics?.nosignal ? `📡 Связи и интернета на месте почти нет\n` : '') +
    // Мед-показания — ДО программы и описания: человек с противопоказанием
    // должен увидеть их раньше, чем решит записаться.
    (ev.logistics?.medical
      ? `\n🩺 <b>Кому нельзя / с чем осторожно</b>\n${esc(String(ev.logistics.medical).slice(0, 500))}\n`
      : '') +
...
function mainMenu(_admin = false) {
   ...
   */
  // Вход в приложение — ТОЛЬКО синяя кнопка слева от поля ввода
  // (setChatMenuButton). Дублировать её в клавиатуре нельзя: получались две
  // одинаковые кнопки «Открыть FLINT», и человек не понимал, чем они
  // отличаются. В клавиатуре остаётся то, чего в приложении нет по смыслу:
  // SOS (нужен в секунду, когда открывать приложение некогда) и свёрнутые
  // инструменты организатора.
  const rows: any[][] = [[{ text: '🆘 SOS' }]];
  if (_admin) rows[0].push({ text: '⚙️ Организатору' });
...
async function sendWelcome(chatId: number, openBtn: any, isCoreUser = false, isOrgUser = false) {
  ...
  lines.push('', 'Афиша, запись, логистика и профиль — в приложении: кнопка <b>«Открыть FLINT»</b> слева от поля ввода.');
...
async function sendEventCard(chatId: number, ev: any, site: string, extraButtons: any[][] = []) {
  ...
  const hasImage = !!(telegramImage || ev.image);
  const ts = Date.now();
  ...
  const caption =
    `<b>${esc(ev.title)}</b>${whenStr ? ` · ${esc(whenStr)}` : ''}` +
    (ev.location ? `\n📍 ${esc(ev.location)}` : '');
  const markup = kb([
    [{ text: '📋 Подробнее', web_app: { url: `${site}/?ev=${encodeURIComponent(ev.id)}` } }],
    ...extraButtons,
  ]);
  const sent = hasImage
    ? await tg('sendPhoto', { chat_id: chatId, photo, parse_mode: 'HTML', caption, reply_markup: markup })
    : null;
  if (!sent || (sent as any).ok !== true) {
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (update.callback_query) {
       ...
       */
      if (data === 'usreply') {
        // answerCallbackQuery + sendMessage ДО setSession: если сессия упадёт
        // (БД не готова), сообщение всё равно дойдёт, а ответ не потеряется —
        // следующий текст пользователя попадёт в support_text через getSession.
        await tg('answerCallbackQuery', { callback_query_id: cq.id });
        await tg('sendMessage', {
          chat_id: chatId, parse_mode: 'HTML',
      ...
      // «Позвать друга»: показываем карточку. Текстовая ссылка в caption переживает
      // пересылку (в отличие от inline-кнопок). Друг получает афишу+текст+ссылку.
      if (data.startsWith('share_')) {
        ...
        const caption =
          ...
          (ev.price_type === 'free' || !ev.price_type
            ...
            : ev.price_label ? `💳 ${esc(ev.price_label)}\n` : `💳 Каждый платит за себя\n`) +
          `\n🔗 <a href="${esc(botLink)}">Открыть и забронировать</a>\n\n` +
          `📤 Перешли эту карточку другу — ссылка в тексте сохранится.`;

        // Программа · Правила · Забронировать место — для тебя (кнопки не пересылаются).
        const buttons: any[] = [];
        const progRow: any[] = [];
    ...
    if (msg && typeof msg.text === 'string') {
      ...
      if (msg.chat.type === 'group' || msg.chat.type === 'supergroup') {
        ...
        let aiUsed = false;
        /**
         * Предложение подвезти. «могу забрать вас» сюда не попадало: было
         * только «заберу». Живой случай — «С метро Уручье могу забрать вас»:
         * человек назвал новую точку старта, а в машине так и осталась
         * прежняя, и попутчики поехали бы не туда.
         */
        const carOffer = /(?<![а-яёa-z])(каршеринг|каршер|на\s+своей|на\s+машине|на\s+авто|за\s+рулём|за\s+рулем|подвезу|заберу|захвачу|могу\s+(взять|забрать|захватить|подвезти|подбросить|подкинуть))/i;
        ...
        if (linkedEvent && msg.from?.id && seatNeed.test(text) && text.length > 10) {
          ...
          try {
            ...
            } else if (carOffer.test(text)) {
              /**
               * Формулировка просьбы, смысл предложения: «могу подкинуть вас
               * от метро» попадает в seatNeed словом «подкинуть». ИИ сказал
               * «не просьба» — отдаём сообщение разбору машины ниже, иначе
               * предложение потерялось бы молча. Второй вызов ИИ здесь
               * допустим: maxDuration функции 60 с (vercel.json), а случай
               * редкий — нужно совпадение обоих паттернов сразу.
               */
              aiUsed = false;
            }
          } catch {
        ...
        /**
         * ВОПРОС ЧЕЛОВЕКУ, А НЕ БОТУ — бот молчит.
         * Живой случай: Людмила спросила Егора «А ты где живёшь?», и бот влез
         * с «в данных поездки не указано место проживания Егора». Вопрос был
         * не к нему, ответ бесполезный, а доверие к боту в чате тратится
         * быстро: после пары таких его перестают читать вовсе.
         * Раньше ловилось только обращение по имени («Даня, ты выехал?»).
         * Добавлены два признака: второе лицо («ты», «у тебя») — в оргвопросе
         * к боту пишут «во сколько выезд», а не «ты где»; и ответ на чужое
         * сообщение — там разговор уже идёт между двумя.
         */
        const addressedToPerson = /^@?[А-ЯЁ][а-яё]{2,}\s*[,!]/.test(text.trim())
          || /(?<![а-яёa-z])(ты|тебя|тебе|тобой|твой|твоя|твои|тво[ёе])(?![а-яёa-z])/i.test(text);
        const replyToPerson = Boolean(msg.reply_to_message && !msg.reply_to_message.from?.is_bot);
        if (linkedEvent && looksLikeQuestion && orgTopic.test(text) && !addressedToPerson && !replyToPerson && text.length > 8 && !aiUsed) {
      ...
      if (!text.startsWith('/')) {
        ...
        if (sess && sess.state === 'exp_add') {
          ...
          try {
            // Инлайн вместо import('../_lib/ai-helpers') — Vercel не бандлит
            // файлы из папок с «_» в рантайме, динамический import ронял
            // MODULE_NOT_FOUND (тихо, из-за try/catch, но ИИ-разбор не работал).
            const parsed = await geminiJSON(
              `Раздели текст на ОТДЕЛЬНЫЕ покупки. КАЖДАЯ покупка = название + сумма.\n` +
              `Текст: "${raw.slice(0, 400)}"\n` +
