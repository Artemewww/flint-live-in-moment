...
export default function CalendarGrid({ events, selectedEventId, onSelectEvent, onOpenDetail }: CalendarGridProps) {
  ...
  // Главный календарь: клик по дню всегда показывает псписок событий под сеткой.
  // Если одно — тоже через selectedDayEvents (пользователь выбирает и переходит).
  // Исключение: если ровно 1 событие, сразу открываем карточку (удобнее).
  ...
  /** Компактная карточка события для списков (переиспользуется и в главной, и в годовой). */
  ...
  return (
    <div className="space-y-3 font-sans">
      ...
      <div className="relative">
        ...
        >
          ...
          {Array.from({ length: daysInMonth - todayDayNum + 1 }, (_, idx) => {
            ...
            // Совпадает ли поповер с этим днём
                ...
                {(() => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Только цветные точки-маркеры, без текста */}
                      {hasEvents ? (
        ...
        {/* Яркая стрелка-указатель от активной ячейки к поповеру */}
        {selectedDayEvents && selectedDayEvents.length > 1 && (
          <div className="flex justify-center mt-0">
            <div className="w-0 h-0 border-l-[10px] border-r-[10px] border-b-[10px] border-l-transparent border-r-transparent border-b-brand/60" />
          </div>
        )}
      </div>
      ...
      {selectedDayEvents && selectedDayEvents.length > 1 && (
        <div className="bg-[#161616] border border-brand/20 rounded-2xl p-4 relative">
          {/* Header */}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-3 sm:p-4 bg-black/95 backdrop-blur-md" id="all-year-plan-modal">
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
              ...
              <div>
                <div className="grid grid-cols-4 sm:grid-cols-7 gap-2.5">
                  ...
                  {Array.from({ length: activeYearMonth.days }, (_, idx) => {
                    ...
                    // Подсветка активной ячейки (выбранный день)
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          if (!hasEvt) return;
                          // Показываем список событий дня прямо внутри годового модала
              ...
              {/* Список событий выбранного дня (раскрывается внутри модала) */}
                ...
                <div className="space-y-3 animate-fadeIn">
                  <div className="flex items-center justify-between">
                    <h4 className="text-[10px] uppercase font-mono tracking-widest text-brand font-black">
                      {yearModalDayEvents[0].dateLabel || yearModalDayEvents[0].date} · {yearModalDayEvents.length} {yearModalDayEvents.length === 1 ? 'событие' : 'события'}
                    </h4>
                    <button
