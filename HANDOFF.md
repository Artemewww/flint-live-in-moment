...
### ЗАДАЧА: Мультиточечные события / «маршрут дня» (СПРОЕКТИРОВАНО 15.07, не начато)
Кейс владельца: день рождения — вейкборд на Птичи → кафе → кино. Несколько точек в один день, у каждой своя оплата, общий маршрут, уведомления при изменениях.

**Проектные решения (на существующих структурах, БЕЗ DDL — не ломаем код):**
1. **Модель**: `events.logistics.itinerary` (колонка logistics jsonb уже есть, сохраняется через POST-upsert и PATCH admin/events — PATCH уже умеет logistics):
   `[{ id, title, time, durationMin, lat, lng, address, payment: 'self'|'host'|'split'|'free', price, priceNote, note }]`
   Порядок массива = порядок движения по точкам.
2. **Маршрут**: одна кнопка «🧭 Маршрут в Яндексе» — URL собирается из точек: `https://yandex.ru/maps/?rtext=lat1,lng1~lat2,lng2~...&rtt=auto`. Работает и в боте, и на сайте.
3. **Бот, карточка события**: блок «Маршрут дня»: `12:00 Вейк на Птичи — платишь сам (25 BYN)` построчно + итоговая строка «Ты платишь: вейк 25 + кино 10; кафе — за счёт организатора». Кнопка маршрута.
4. **Изменения**: правка точки/цены/плательщика в админке → запись в СУЩЕСТВУЮЩУЮ таблицу `event_changes` + рассылка записанным с кнопкой «✅ Понял» (механизм ack_ уже работает). Не спамить: батчить изменения одним сообщением.
5. **Напоминания**: cron (уже 9:00 daily) в ДЕНЬ события шлёт записанным таймлайн дня одним сообщением. Не назойливо: одно сообщение, только если itinerary непуст.
6. **Админка**: ItineraryEditor в формах события (по образцу ListEditor): точка = title/time/оплата(4 кнопки)/цена/координаты (вставка «lat, lng» из карт — уже есть паттерн coordinates), стрелки порядка, удалить. Кнопка «↻» на точке — ИИ перегенерирует ТОЛЬКО эту точку (промпт: контекст события + соседние точки).
7. **ИИ-автогенерация события** (aiStage-флоу в AddEventModal + api/ai.ts): если в запросе несколько активностей — ИИ возвращает itinerary[] (schema: массив объектов выше) вместе с обычными полями. Owner: «качества из базы тоже использовать» — houseQualities учитывать в промпте.
8. **Кто платит** — семантика: self=каждый сам, host=организатор/именинник платит за всех, split=делится поровну (сумма/кол-во записанных), free=бесплатно. Смена host↔self → обязательное уведомление (п.4).
