...
## ⚠️ СНАЧАЛА СДЕЛАТЬ РУКАМИ (иначе часть функций молча не работает)

1. **Выполнить `supabase/migrations/2026-final.sql`** в Supabase SQL Editor.
   Без него не будут работать: отзывы, голосование за программу, «мне интересно»,
   чек-лист готовности, явка/баллы, «под вопросом», заместитель, атомарная бронь
   места в машине. Код это переживает без падений, но данные не сохраняются.
   Скрипт идемпотентный — можно запускать повторно.

2. **Задать `CRON_SECRET`** в Vercel Env — крон напоминаний (`api/cron/reminders.ts`)
   иначе защищён только `ADMIN_TOKEN`.

3. **Проверить деплой.** Функций сейчас 11 из 12. Если добавишь 13-ю — деплой упадёт
   молча, а прод продолжит отдавать старую сборку (см. §9).
