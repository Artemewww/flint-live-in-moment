...
## 11. V2 — форматы событий: офлайн / онлайн / гибрид  ⬜ (записано 12.07.2026)

**Проблема (реальный кейс «Смешанный Круг» — детокс/голодание):** событие онлайн,
но карточка навязывает офлайн-поля: «РАЙОН · точка сбора скрыта 🔒», логистика
машин/палаток, взнос на бензин, .ics с местом. Для онлайна это мусор. Нужен
общий механизм: событие может быть **локальным, онлайн или гибридным**, и поля
подстраиваются под формат.

### Модель данных
- Новое поле `events.format text default 'offline'` → `offline | online | hybrid`.
  ⚠️ НЕ путать с `kind` — тот уже занят обобщением rides (машины/палатки).
- Онлайн: вместо локации — блок «Пространство встречи»: ссылка на группу/канал
  события (`events.group_link text`). Скрываем: точку сбора, карту, логистику,
  «взнос на бензин», палатки, гейтинг локации.
- Гибрид: показываем И локацию, И ссылку на чат.
- Условный рендер по `format` в `EventDetailModal.tsx` и в боте (`logi_`/`org_`
  прятать для онлайна).

### Группа под событие — техническая правда Bot API (важно, чтобы не обещать невозможного)
- ⚠️ Бот **не может** создать Telegram-группу/канал программно (Bot API не умеет)
  и **не может** произвольно добавлять людей в чужую группу.
- Рабочая схема («авто-добавление» → «авто-приглашение»): орг один раз создаёт
  группу, делает бота админом, вставляет invite-ссылку в поле события. При
  подтверждённой регистрации бот шлёт участнику персональный инвайт в личку.
- Альтернатива без внешней группы: «виртуальный круг» внутри самого бота —
  дневной чек-ин + пересылка сообщений между участниками (дороже в разработке).

### Контроль по дням / взаимная мотивация
- Расширить `api/cron/reminders.ts`: для онлайн-события с многодневной программой
  слать в группу дневной чек-ин («День 2: как самочувствие? Отметьтесь 🔥»).
- Программа уже группируется по дням (сделано в §7-adjacent) — переиспользовать
  как расписание для авто-постов.

### Мелкие баги карточки (пофиксить при доработке — дёшево)
- «Забронировано 0 из **150%**» — лишний `%` после `maxParticipants` в блоке мест.
- Время дублируется: «17–20 июля, 1800» и ниже ещё раз отдельной строкой «1800».
- Для онлайна «СОСТОЯНИЕ МЕСТ» уместнее как «Участников» (без брони мест/палаток).

**Лимит функций:** всё встраивается в существующие роутеры (поле в БД + условный
рендер + ветка в `webhook.ts`/`reminders.ts`). Новых serverless-функций НЕ требует.
