...
## 12. V2 — календарь: seed-события, актуальный месяц, компактность, пересечения  🟨 (записано 12.07.2026)

**Контекст загрузки событий (важно):** `App.tsx` грузит по цепочке ФОЛБЭКОВ, не слиянием:
`/api/events` (Supabase) → `/events.json` → `src/data.ts` (`SHARED_EVENTS` из
`shared/events.data.js`). Как только БД отдаёт хоть одно событие — статичный seed
**перестаёт показываться**.

### 12.1. Годовые события → единый источник в БД  ✅ (сделано 12.07.2026)

**Проделанная работа:**
- ✅ Хардкод `monthsData` удалён из `CalendarGrid.tsx` — годовой календарь строится из `events` (БД)
- ✅ `getShortEventName` заменён на `getShortEventLabel` — читаемые названия в ячейках
- ✅ `handleDayClick`: при 1 событии → сразу карточка; при ≥2 → поповер со списком
- ✅ Шаблоны вынесены в `src/data/eventTemplates.ts`
- ✅ **Стрелка-указатель:** динамическое позиционирование через `data-daynum` + `getBoundingClientRect`
- ✅ **Любой клик по дате** раскрывает нижний блок; `scrollIntoView` к плашке
- ✅ **Годовой модал:** `max-w-[95vw]`/`max-w-7xl`; клик по дню — список ВНУТРИ модала без закрытия
- ✅ **Обложки в EventCard:** превью изображения (или иконка-заглушка) в списках событий
- ✅ **Today pulsing dot** добавлен в годовой календарь
- ✅ **`pluralEvents`** — правильное склонение "событие/события/событий"
- ✅ **Нижний футер модала удалён**, шапка минималистичная
- ✅ **`normalizeTime("1800") → "18:00"`** — хелпер для коррекции слитного времени
- ✅ **AI-генерация обложек** — кнопка «🎨 Сгенерировать обложку» в EditEventModal; промпт формируется из названия + описания с жёсткими стилевыми параметрами
- ✅ `npx tsc --noEmit` — 0 ошибок, `npm run build` — успешно
- ✅ Изменения запушены в origin main

**Два РАЗНЫХ хардкода событий (не путать):**
1. `shared/events.data.js` (`SHARED_EVENTS`) — fallback-список афиши, если `/api/events` пуст.
2. `CalendarGrid.tsx` → `const monthsData = [...]` (~стр. 98) — курированный ГОДОВОЙ
   календарь (Июнь 2026 →), события с полем `day` (день месяца, без полной даты/года).
   Именно это видно в «весь год» и это те «интересные события не из БД».

**Решение (владелец, цель = максимальная автоматизация): подход A — перенести в БД,
один источник правды.** Overlay/merge отклонён (навсегда оставляет хардкод + дубли).

**Шаги реализации (нужен запас лимита + живая проверка превью — НЕ делать на фумах,
файл уже давал белый экран):**
1. **Data-миграция `monthsData` → БД.** Написать одноразовый скрипт/`.sql`, который
   разворачивает каждый `monthsData[i]` в реальные `events`: `day` + месяц блока + год
   → полная `date` (`YYYY-MM-DD`); поля `title/type/description/duration→time`;
   `id` из существующего (`banya-flint-weekly` и т.п.). `insert ... on conflict (id)
   do nothing` — идемпотентно, повторный запуск безопасен. Многодневные (`duration`
   «3 дня», «2-4 дня») → проставить `date_end`. Запускает владелец в Supabase.
2. **Годовой вид читает из `events`.** В `CalendarGrid.tsx` заменить обращения к
   `monthsData[modalActiveMonth]` на группировку РЕАЛЬНОГО `events` по месяцам
   (`evt.date.slice(0,7)`), 12 месяцев года, текущий месяц активен по умолчанию.
   Убрать хардкод-диапазоны дней (§12.4) заодно.
3. **Текущий месяц актуален** автоматически, т.к. данные из БД по датам.
4. **Новые события** через админку сразу попадают в год — единый источник.
- Итог: курированные события сохранены (стали настоящими, редактируются, не исчезают),
  месяц актуален, новые события формируются на год сами. Новых функций НЕ требует.

**Уточнение (владелец, 12.07.2026): «левые события» + шаблоны.**
- В БД сейчас ~5 реальных событий, а годовой `monthsData` показывает больше —
  это и есть «левые события» (хардкод без пары в БД). Годовой вид должен показывать
  ТОЛЬКО реальные события из БД (после шага 2 так и будет: `monthsData` перестаёт
  быть источником).
- Курированные из `monthsData` НЕ выбрасывать, а превратить в **шаблоны событий**:
  отдельная сущность (напр. `event_templates` или JSON-пресеты в коде), из которой
  админ создаёт новое событие в один клик и ставит на нужную дату. Так «Мужская баня
  FLINT», «Сплав на каяках» и т.п. становятся переиспользуемыми заготовками, а не
  фантомами в календаре.
- Порядок: (1) вытащить `monthsData` в файл-пресетов шаблонов; (2) годовой вид на БД;
  (3) в админке кнопка «Создать из шаблона» → выбор пресета → дата → реальное событие.

### 12.2. Актуальный месяц при входе в «весь год»  ✅ (сделано)
- Было: годовой модал открывался на январе (`setModalActiveMonth(0)`).
- Стало: открывается на текущем месяце (`currentMonth`). См. `CalendarGrid.tsx`.

### 12.3. Компактность года на мобиле и десктопе  ✅ (сделано 12.07.2026)
- Сейчас годовой модал занимает слишком много места, неудобен.
- Идея: сетка 3×4 (или 2×6) миниатюр месяцев, текущий месяц подсвечен, точки-маркеры
  дней с событиями; тап по месяцу → раскрытие. Уместить без вертикальной простыни.

**Финальные требования владельца (12.07.2026, по скриншотам):**
- Верхняя шапка модала («ПЛАН МЕРОПРИЯТИЙ / ТРЕЗВОЕ СООБЩЕСТВО…») — убрать или
  сильно сжать, она зря занимает место.
- Нижние кнопки «Закрыть» / «В бот» в годовом модале — лишние, убрать (Закрыть уже
  есть сверху). Освободить место под календарь.
- Сетка календаря — на всю ширину экрана, ячейки шире (сейчас есть боковые поля).
- ГЛАВНОЕ (§12.1): события в годовом виде ДОЛЖНЫ совпадать с реальной БД. Пример
  рассинхрона: на главной в июле 5 реальных событий, а в годовом модале — другие
  (захардкоженные `monthsData`: Слёт/Сплав/Баня/Книга). Год строить из `events` (БД),
  а `monthsData` вынести в шаблоны (см. §12.1). Пока это не сделано, год показывает
  фантомные события.

### 12.4. Пересечения однодневных и многодневных в один день  ⬜
- В `CalendarGrid.tsx` (~строки 34–49) есть ХАРДКОД диапазонов дней
  (`dayNum >= 26 && dayNum <= 28` и т.п.) — источник кривого поведения.
- Нужно: честный расчёт «событий в день» через `date`..`dateEnd` (диапазон), а не
  хардкод. День с несколькими событиями (однодневное + многодневное) → показывать
  стопкой/бейджем «N», тап открывает список за день (`selectedDayEvents` уже есть).
- Убрать хардкод-ветки, заменить на `evt.date <= d && d <= (evt.dateEnd || evt.date)`.

**Лимит функций:** чистый фронт (`CalendarGrid.tsx` + опц. `App.tsx`) или разовый
seed в БД. Новых serverless-функций НЕ требует.
