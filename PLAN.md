...
- ✅ `getShortEventName` заменён на `getShortEventLabel` — в ячейках дней показываются читаемые названия событий (не технические типы "Микс"/"Дзен")
- ✅ `handleDayClick`: при 1 событии → сразу карточка; при ≥2 → компактный поповер со списком всех событий дня
- ✅ Годовой модал: при клике на день с несколькими событиями → открывается поповер на главном экране
- ✅ Шаблоны вынесены в `src/data/eventTemplates.ts`, в админке кнопка «Создать из шаблона» с выбором даты
- ✅ **UI/UX доработка:** из ячеек убраны текстовые названия — только цветные точки-маркеры
- ✅ **Годовой модал без редиректа:** клик по дню раскрывает список событий ВНУТРИ модала, модал не закрывается
- ✅ **Нижний футер удалён:** кнопки «Закрыть» / «В бот» убраны, шапка минималистичная (только заголовок + крестик)
- ✅ **Стрелка-указатель:** яркий треугольник между активной ячейкой и поповером на главной
- ✅ `npx tsc --noEmit` — 0 ошибок, `npm run build` — успешно
- ✅ Изменения запушены в origin main (3 коммита)
