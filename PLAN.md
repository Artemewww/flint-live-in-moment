...
### 12.1. «Интересные события не из БД исчезают»  ⬜
- Причина: красивые демо-события лежат только в seed (`shared/events.data.js`) и
  видны лишь когда API пуст/упал. С реальной БД они пропадают.
- Варианты решения:
  - **(A, рекомендуется)** одноразово залить нужные seed-события в БД (через админку
    или seed-скрипт `insert ... on conflict do nothing`) — станут «настоящими».
  - **(B)** мержить seed + БД в `App.tsx` с дедупом по `id` (seed как «шаблон»,
    БД перекрывает). Риск: демо-события всплывут у всех, нужен флаг «показывать шаблонные».
- Решить, какие именно события ценны, и выбрать A или B.
